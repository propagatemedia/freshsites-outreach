...
def score_demo(html: str, base: str, expected_name: str, vertical: str):
    ...
    if '£997' in html or 'premium' in low: fail(0.8,'Premium tier should be removed from this flow')
    if 'schedule a discovery call' not in low or 'https://propagate.media/schedule-call' not in html: fail(0.8,'missing Schedule a Discovery Call option')
    if 'images are for demo purposes only' not in low: fail(0.6,'missing demo image disclaimer')
