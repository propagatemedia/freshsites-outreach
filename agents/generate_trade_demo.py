...
No external images. Uses local approved assets and CSS icon cards so quality does
not depend on random stock photos.
"""
...
IMG_BASE = "../assets/img/"
PROPAGATE_EMAIL = "hello@propagate.media"
...
VERTICALS = {
    "electrician": {
        ...
        "proof": ["NICEIC / qualified credentials", "Clear call-out areas", "Tap-to-call on mobile", "Quote request form"],
        "accent": "#f5b301",
...
def vertical_key(data: dict) -> str:
    ...
    if any(k in text for k in ["electric", "electrical"]): return "electrician"
    if any(k in text for k in ["roof", "leadwork"]): return "roofing"
    if any(k in text for k in ["plumb", "heating", "boiler", "hvac"]): return "plumbing"
    if any(k in text for k in ["supply", "supplies", "store"]): return "supply"
    if any(k in text for k in ["clad", "construction", "builder", "carpenter"]): return "construction"
    return "construction"
...
*{{box-sizing:border-box}}body{{margin:0;font-family:Inter,system-ui,-apple-system,Segoe UI,sans-serif;color:var(--ink);background:var(--bg);line-height:1.6;padding-bottom:82px}}a{{color:inherit;text-decoration:none}}.wrap{{max-width:1120px;margin:0 auto;padding:0 22px}}.top{{background:var(--dark);color:#fff;font-size:.92rem}}.top .wrap{{display:flex;justify-content:space-between;gap:16px;padding-top:10px;padding-bottom:10px;flex-wrap:wrap}}header{{background:#fff;border-bottom:1px solid var(--line);position:sticky;top:0;z-index:50}}nav{{display:flex;align-items:center;justify-content:space-between;padding:18px 0;gap:22px}}.brand{{font-weight:900;font-size:1.2rem;letter-spacing:-.03em}}.links{{display:flex;align-items:center;gap:24px;font-weight:750}}.btn{{display:inline-flex;align-items:center;justify-content:center;border-radius:999px;padding:12px 20px;font-weight:850;border:2px solid var(--brand);cursor:pointer;color:#fff !important;background:var(--brand);box-shadow:0 10px 24px rgba(0,0,0,.12)}}.btn.secondary{{background:#fff;color:var(--brand)!important}}.hero{{position:relative;min-height:620px;color:#fff;overflow:hidden;background:var(--dark)}}.hero img{{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center;filter:saturate(.9)}}.hero:after{{content:"";position:absolute;inset:0;background:linear-gradient(90deg,rgba(16,19,24,.88),rgba(16,19,24,.72) 50%,rgba(16,19,24,.25))}}.hero-in{{position:relative;z-index:1;min-height:620px;display:grid;align-items:center;max-width:760px}}.eyebrow{{color:var(--brand);text-transform:uppercase;letter-spacing:.16em;font-size:.78rem;font-weight:950}}h1{{font-size:clamp(2.35rem,6vw,5rem);line-height:.98;margin:12px 0 18px;letter-spacing:-.06em}}.lead{{font-size:1.22rem;color:#e5e7eb;max-width:680px}}.hero-actions{{display:flex;gap:14px;margin-top:28px;flex-wrap:wrap}}.section{{padding:82px 0}}.grid{{display:grid;grid-template-columns:repeat(3,1fr);gap:20px}}.service-card,.audit-card,.contact-card{{background:#fff;border:1px solid var(--line);border-radius:22px;padding:26px;box-shadow:0 14px 34px rgba(15,23,42,.07)}}.service-card{{transition:.2s transform,.2s box-shadow}}.service-card:hover{{transform:translateY(-4px);box-shadow:0 20px 45px rgba(15,23,42,.12)}}.svc-icon{{width:54px;height:54px;border-radius:16px;background:color-mix(in srgb,var(--brand) 14%,#fff);display:flex;align-items:center;justify-content:center;font-size:1.5rem;margin-bottom:16px}}h2{{font-size:clamp(2rem,4vw,3rem);letter-spacing:-.04em;line-height:1.08;margin:8px 0 14px}}h3{{margin:0 0 8px;font-size:1.18rem}}p{{margin:0 0 14px}}.split{{display:grid;grid-template-columns:1fr 1fr;gap:36px;align-items:center}}.audit-card{{background:var(--dark);color:#fff}}.audit-card p{{color:#d7dce5}}.audit-card ul{{margin:18px 0 0;padding-left:20px;color:#d7dce5}}.contact{{background:#fff}}.contact-grid{{display:grid;grid-template-columns:1.1fr .9fr;gap:28px}}label{{display:block;font-weight:800;margin-bottom:6px}}input,textarea,select{{width:100%;border:2px solid var(--line);border-radius:12px;padding:14px;font:inherit;margin-bottom:14px}}textarea{{min-height:132px}}.success{{display:none;background:#ecfdf5;border:2px solid #86efac;border-radius:16px;padding:20px;color:#065f46}}iframe{{width:100%;height:280px;border:0;border-radius:18px;margin-top:14px}}footer{{background:var(--dark);color:#cbd5e1;text-align:center;padding:36px 20px}}.buy-bar{{position:fixed;bottom:0;left:0;right:0;background:#fff;border-top:2px solid var(--brand);padding:14px 20px;z-index:100;display:flex;justify-content:center;align-items:center;gap:18px;box-shadow:0 -4px 24px rgba(0,0,0,.12);flex-wrap:wrap}}.buy-bar span{{font-weight:900;color:#111}}.tier-panel{{display:none;position:fixed;left:0;right:0;bottom:0;background:#fff;border-top:3px solid var(--brand);z-index:110;padding:34px 24px 100px;box-shadow:0 -18px 60px rgba(0,0,0,.2)}}.tier-panel.active{{display:block}}.tiers{{max-width:930px;margin:22px auto 0;display:grid;grid-template-columns:repeat(3,1fr);gap:18px}}.tier{{border:1px solid var(--line);border-radius:18px;padding:22px;background:#fafafa}}.tier b{{font-size:1.8rem}}.confirm-overlay{{display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:120;align-items:center;justify-content:center;padding:20px}}.confirm-overlay.active{{display:flex}}.confirm-box{{background:#fff;border-radius:24px;padding:30px;max-width:460px;text-align:center}}@media(max-width:850px){{.links a:not(.btn){{display:none}}.grid,.split,.contact-grid,.tiers{{grid-template-columns:1fr}}.hero,.hero-in{{min-height:540px}}}}
</style>
</head>
<body>
...
<div class="tier-panel" id="tp"><div style="max-width:930px;margin:auto;position:relative"><button style="position:absolute;right:0;top:0;border:0;background:none;font-size:2rem;cursor:pointer" onclick="hideTiers()">×</button><h2 style="text-align:center">Get Your Site</h2><p style="text-align:center;color:var(--muted)">Pick the package that fits. No monthly fees. No lock-in.</p><div class="tiers"><div class="tier"><h3>Buy Outright</h3><b>£149</b><p>Own the page. Host it anywhere.</p><a class="btn" href="mailto:{PROPAGATE_EMAIL}?subject=Buy%20Outright%20-%20{esc(slug)}">Start</a></div><div class="tier"><h3>Hosted + Edits</h3><b>£399</b><p>12 months hosting and two edit rounds.</p><a class="btn" href="mailto:{PROPAGATE_EMAIL}?subject=Hosted%20Package%20-%20{esc(slug)}">Start</a></div><div class="tier"><h3>Premium</h3><b>£997</b><p>Priority build, edits and automation options.</p><a class="btn" href="mailto:{PROPAGATE_EMAIL}?subject=Premium%20Package%20-%20{esc(slug)}">Start</a></div></div></div></div>
